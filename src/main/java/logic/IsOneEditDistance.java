 ...
 * <p>
 * Insert exactly one character into s to get t. Delete exactly one character from s to get t. Replace exactly one
 * character of s with a different character to get t.
 */
    ...
    public boolean isOneEditDistance(String s, String t) {
        ...
        while (i < sLen && j < tLen) {
            if (s.charAt(i) == t.charAt(j)) {
                ...
                if (sLen == tLen) {
                    i++;
                    j++; //if their length is same then move both forward
                } else if (sLen > tLen) {
