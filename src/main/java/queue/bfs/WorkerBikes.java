 ...
 * and assign the bike to that worker. (If there are multiple (worker, bike)
 * pairs with the same shortest Manhattan distance, we choose the pair with the smallest worker index;
...
public class WorkerBikes {
    ...
    /**
     * Sort by distance , worker and bike using PQ
     * then start assigning workers bikes based on the sort condition
     *
     * problem is that the problem executes M*N * (logN) time.
     *
     * can not choose a BFS solution here as that would not cater to min distance , A bike which is closest
     * to a person (person A)  maybe closer to another person (person B).
     * @param workers
     * @param bikes
     * @return
     */
