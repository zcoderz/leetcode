 ...
 * <p>
 * The number of "bulls", which are digits in the guess that are in the correct position. The number of "cows", which
 * are digits in the guess that are in your secret number but are located in the wrong position. Specifically, the
 * non-bull digits in the guess that could be rearranged such that they become bulls. Given the secret number secret and
 * your friend's guess guess, return the hint for your friend's guess.
 * <p>
 ...
 * Example 1:
 * <p>
 * Input: secret = "1807", guess = "7810" Output: "1A3B" Explanation: Bulls are connected with a '|' and cows are
 * underlined: "1807" | "7810" Example 2:
 * <p>
 * Input: secret = "1123", guess = "0111" Output: "1A1B" Explanation: Bulls are connected with a '|' and cows are
 * underlined: "1123"        "1123" |      or     | "0111"        "0111" Note that only one of the two unmatched 1s is
 * counted as a cow since the non-bull digits can only be rearranged to allow one 1 to be a bull.
 */
public class BullsAndCows {
    ...
    /**
     * Accessing integers by their indices in an array is much faster than using a hash map the below code runs leetcode
     * test cases in 1 ms whereas the one above runs in 6ms.
     *
     * @param secret
     * @param guess
     * @return
