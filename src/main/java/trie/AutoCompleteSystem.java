 ...
 * what we get is quick retrieval of the top n sentences at each node.
 * however , this comes at a cost where we are storing a lot of data at each trienode.
 *
 * we could have a background job that updates the trie data by removing unneeded data
 * we could also simplify the trie structure to just contain the top n strings , rather than hold them in a set and map
...
public class AutoCompleteSystem {
    ...
    /**
     * this trie stores regular character to sub trie mappings
     * and in addition stores a set of SentenceCount. This sentence is sorted by weight and then
     * alphabetically so that it can return the appropriate sentences
     *
     * when a sentence's frequency is being updated we need to remove and then add it from the set
     * therefore we use a map that maps given string to the sentence
     */
