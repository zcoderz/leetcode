 ...
 * unclear. This question is asked a whole lot by Amazon. I guess they are trying to check if candidate can
 * ask questions to clear out the ambiguity in a spec.
 *
 * if asked this question, id ask clear questions to the interviewer to clarify the question.
 *
 * see question wording from leet code below
...
//        String [] sites = {"home","about","career","home","cart","maps","home","home","about","career"};
    ...
    /**
     * main caller method for the question.
     * the code for each user finds combination of sites (3 sites) that the person visited
     * the combinations are permuted based on the sort of the times stamp that the site was visited.
     *
     * then the list of sites across each users , where list consists of 3 sites is aggregated.
     * when a site combination repeats , the count for that combination is increased.
     *
     * the site combination with highest frequency is returned. if two combs have same frequency one
     * with lower lexical sort order is returned
     * @param username
     * @param timestamp
     * @param website
     * @return
    ...
    public List<String> mostVisitedPattern(String[] username, int[] timestamp, String[] website) {
        ...
        data.add(tp.first); data.add(tp.second); data.add(tp.third);
        return data;
    }
