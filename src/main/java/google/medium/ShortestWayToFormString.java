...
public class ShortestWayToFormString {
    ...
    /**
     * simple approach :
     *   - first check whether transform is possible
     *   - then run source to target by moving source indexes ahead.
     *   (could optimize further by recording sub arrays in target that required a single pass on source
     *   thus if the pattern repeats, we just move source ahead by that many indexes.
     *   this would be advantageous in case of large repetitive patterns but infact slow if patterns are distinct)
     * @param source
     * @param target
     * @return
     */
